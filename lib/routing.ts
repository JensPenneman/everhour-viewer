 ...
 * view is reflected in the address bar so views are deep-linkable, shareable,
 * and survive refresh / back-forward. All routing is path-based and served by
 * the optional catch-all route, so any of these paths resolves to the shell:
 *
 *   /                         → today (live tracking; welcome on first run)
...
/** Parse a pathname into the view it should render. Unknown paths → today. */
export function parseRoute(pathname: string): ViewerRoute {
  const segs = pathname.split("/").filter(Boolean);
  if (segs.length === 0) return { view: "today" };
 ...
 * direction: today (0) → week|profile (1) → day (2).
 */
export function routeDepth(route: ViewerRoute): number {
  if (route.view === "today") return 0;

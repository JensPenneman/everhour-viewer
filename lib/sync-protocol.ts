/**
 * Wire format for the streaming `/api/sync` endpoint — the shared contract
 * between the server orchestrator (producer) and the client sync hook
 * (consumer).
 *
 * It lives in `lib/` (not `server/`) precisely so the client can import these
 * types without reaching into `@/server/*`. The server emits a sequence of
 * NDJSON events. Order:
 *
 *   1. `profile`   — once, immediately after the API key is validated.
 *   2. `plan`      — once, after the timesheet list is fetched. Tells the
 *                    client how many weeks will be fetched vs. skipped.
 ...
 *
 * The `error` event is also emitted mid-stream if the upstream Everhour API
 * fails after some weeks have already streamed: any data already sent is
 * valid; the client merges what it received and surfaces the error.
 */

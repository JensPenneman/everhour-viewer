  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-full px-6 py-12">
      <div className="max-w-md w-full">
        ...
        <p className="text-muted text-[14px] mb-7 text-center">
          Bekijk je Everhour tijdsregistraties lokaal. Je gegevens blijven in deze browser bewaard —
          niets wordt gedeeld.
        </p>

        <div className="bg-panel border border-border rounded-xl p-5 mb-3">
          <Step
            n={1}
        ...
        <div className="text-center text-[12px] text-muted">
          ...
          <button type="button" onClick={onLoad} className="text-accent hover:underline">
...
function Step({ n, title, description, action, done }: StepProps) {
  return (
    <div className="flex gap-3.5 items-start">
      <div
        className={`mt-0.5 w-6 h-6 rounded-full flex items-center justify-center text-[12px] font-semibold shrink-0 ${
          done ? "bg-good-bg text-good" : "bg-accent-bg text-accent"
        }`}
      >
        {done ? "✓" : n}
      </div>
      <div className="flex-1">
        ...
        <div className="text-[12.5px] text-muted mb-2.5 leading-relaxed">{description}</div>
        <div>{action}</div>
      </div>
    </div>
  );
